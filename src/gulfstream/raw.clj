...
(defn frames [d & args]
  ...
  (let [c (count d)
        op (some #{:op} args)
        h (mask {:size c :text? (if (string? d) true) :final? (or (some #{:final?} args)true) :op? op})
        ...
        _ (System/arraycopy ba 0 f 0 (count ba)) ;;use map cat!
        _ (System/arraycopy d 0 f (count ba) (count d))]
    f))

(defn read-frames [b]
  (if-let [h (read-mask (first b))]
